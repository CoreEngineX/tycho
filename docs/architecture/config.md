...
```mermaid
flowchart TB
  subgraph RULES["rules, deepest last"]
    direction LR
    A["watch<br/>~/Developer/CoreEngineX"]
    D["ignore<br/>.../scratch"]
    E["watch<br/>.../scratch/keep"]
    A --> D --> E
  end
  subgraph OUT["resulting decision"]
    direction LR
    F1["org/notes.md<br/>captured, deepest is the watch on the root"]
    F2["scratch/tmp.bin<br/>skipped, deepest is the ignore"]
    F3["scratch/keep/key.pem<br/>captured, deepest is the inner watch"]
  end
  RULES --> OUT
```
